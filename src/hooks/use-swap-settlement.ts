...
export function useSwapSettlement(): UseSwapSettlementResult {
	...
	const [status, setStatus] =
		useState<UseSwapSettlementResult["status"]>("idle");
	...
	const execute = useCallback<UseSwapSettlementResult["execute"]>(
		async (params) => {
			...
			try {
				const decimals =
					params.fromTokenDecimals ?? (params.fromTokenAddress ? 18 : 6);
			...
			try {
				const settlement = await (prepareSettlement as any)({
				...
				setStatus("signing");
				const signed = await (signAuthorization as any)(
					wallet as any,
					settlement,
				);

				setStatus("submitting");
				const result = await (settleWithFacilitator as any)(
				...
				setStatus("error");
				const err = new Error(e?.message || "swap_failed");
				setLastError(err);
